...
const PAGE = [
    ...
    '<div id="toast"></div>',
    '<script>',
    'var TOKEN=localStorage.getItem("play_token")||"";var SEL="";var TT=0;var LOCKS=15;var PHASE="off";var BAL=0;',
    'var NAN=null;var revealedGame=0;var paperY=0;var dragging=false;var dragStartY=0;var dragBaseY=0;',
    ...
    // tờ giấy: kéo xuống bằng chuột/chạm
    'function initPaper(){var p=document.getElementById("paper");',
    'p.addEventListener("pointerdown",function(e){dragging=true;dragStartY=e.clientY;dragBaseY=paperY;p.setPointerCapture(e.pointerId)});',
    'p.addEventListener("pointermove",function(e){if(!dragging)return;var h=document.getElementById("stage").offsetHeight;paperY=Math.max(0,Math.min(h+10,dragBaseY+(e.clientY-dragStartY)));p.style.transform="translateY("+paperY+"px)";p.style.opacity=String(1-paperY/(h+10)*0.35);if(paperY>=h*0.95)revealDone()});',
    ...
    'function resetPaper(){paperY=0;var p=document.getElementById("paper");p.style.transform="translateY(0)";p.style.opacity=1}',
    ...
    'else if(PHASE==="wait"){stt.textContent="⏳ Đang mở bát...";cap.textContent=""}',
    ...
    'box.innerHTML=list.map(function(m){var t=new Date(m.ts);var hh=String(t.getHours()).padStart(2,"0")+":"+String(t.getMinutes()).padStart(2,"0");return \'<div class="cmsg"><b>\'+esc(m.name)+"</b>: "+esc(m.text)+\'<span class="ct">\'+hh+"</span></div>"}).join("");',
