        ...
        try {
            ...
            if (path.startsWith('/api/')) {
                ...
                if (ctx.stock && req.method === 'POST' && path === '/api/stock/open') {
                    ...
                    const r = ctx.stock.open(userId, side, Math.floor(Number(body.amount) || 0), shares);
                    if (r.error) return sendJSON(res, 400, { ok: false, error: r.error });
...
const PAGE = [
    ...
    // ---- 📈 cổ phiếu ----
    // Giá là thứ to nhất trang; đồ thị NẾN (mỗi nhịp 30s một cây) vẽ bằng SVG từ
    // mảng nến server gửi. Hai nút MUA/BÁN đặt cạnh nhau, xanh/đỏ như bàn thật.
    ...
    '#skChart{width:100%;height:150px;display:block;margin-top:10px}',
    ...
    '#skQuick button.on{background:#2a2110;border-color:var(--gold);color:var(--gold)}',
    '#skMoney{background:#12141a;border:1px solid var(--line);border-radius:12px;padding:11px;margin-top:9px;text-align:center}',
    '#skMoney .v{font-size:25px;font-weight:800;letter-spacing:-.02em;font-variant-numeric:tabular-nums;color:var(--gold)}',
    '#skMoney .t{font-size:11.5px;color:var(--muted)}',
    '#skGo{display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-top:10px}',
    '#skGo button{padding:14px 4px;border-radius:12px;font-size:13px;font-weight:800;line-height:1.3;border:0}',
    ...
    '<div class="card">',
    '<div id="skHead">',
    '<div><div class="muted" style="font-size:12px">CỔ PHIẾU DOGCOIN · DOG · M30s</div>',
    '<div id="skPrice">-</div></div>',
    '<div style="text-align:right"><span id="skChg" class="skchip y">-</span>',
    '<div class="muted" style="font-size:12px;margin-top:5px">nến sau <b id="skNext">-</b></div></div>',
    '</div>',
    '<svg id="skChart" viewBox="0 0 300 150" preserveAspectRatio="none"></svg>',
    '<div class="muted" style="display:flex;justify-content:space-between;font-size:11.5px">',
    '<span id="skRange">30 phút gần nhất</span><span>mốc gốc <b id="skBase">1.000</b></span></div>',
    ...
    '<div class="skkv" id="skBurnRow" style="display:none"><span class="muted">⚠️ Cháy cọc nếu giá lên tới</span><b class="skn" id="skBurn">-</b></div>',
    ...
    '<div class="card">',
    '<div class="row"><span class="muted" style="font-size:12px">VÀO LỆNH</span><span class="muted" style="font-size:12px">mua <b id="skAsk">-</b> · bán <b id="skBid">-</b></span></div>',
    '<div id="skSeg"><button id="skModeL" class="on" onclick="skMode(1)">Theo khối lượng</button><button id="skModeM" onclick="skMode(0)">Theo Dogcoin</button></div>',
    '<input id="skInp" type="number" step="0.1" min="0.1" placeholder="Khối lượng (vd 0.1)" oninput="skPrev()">',
    '<div id="skQuick"><button onclick="skQ(0.1)">0.1</button><button onclick="skQ(0.5)">0.5</button><button onclick="skQ(1)">1</button><button onclick="skQ(2)">2</button><button onclick="skQ(-1)">MAX</button></div>',
    '<div id="skMoney"><div class="t" id="skMoneyT">SỐ DOGCOIN PHẢI BỎ RA</div><div class="v" id="skPvC">0</div>',
    '<div class="t" id="skPvS">-</div></div>',
    '<div class="skkv" style="margin-top:8px"><span class="muted">Hoà vốn ở giá</span><b class="skn" id="skPvE">-</b></div>',
    ...
    '<div class="muted" style="font-size:11.5px;margin-top:8px"><b>MUA</b> thắng khi nến đi lên, <b>BÁN</b> thắng khi nến đi xuống - chọn một chiều, muốn đổi thì đóng lệnh cũ trước. Lệnh <b>BÁN</b> phải cọc đúng bằng giá trị lệnh và <b>cháy cọc</b> nếu giá lên gấp đôi giá vào. Chênh mua–bán <b id="skSpr">2</b>% mỗi chiều là phí sàn. Còn <b id="skLeft">-</b> khối lượng, mỗi người tối đa <b id="skPer">-</b>.</div>',
    '</div>',
    ...
    '',
    // ===== 📈 SÀN CỔ PHIẾU DOG =====
    // SKM=1 nhập theo LOT (như bàn giao dịch thật), 0 nhập theo số Dogcoin.
    // SOFF bù lệch đồng hồ máy người chơi so với server, như bên nghiện.
    ...
    'function skMode(m){SKM=m;$("skModeL").classList.toggle("on",m===1);$("skModeM").classList.toggle("on",m===0);',
    '$("skInp").step=m?"0.1":"1";$("skInp").min=m?"0.1":"1";',
    '$("skInp").placeholder=m?"Khối lượng (vd 0.1)":"Số Dogcoin muốn xuống";$("skInp").value="";',
    '$("skMoneyT").textContent=m?"SỐ DOGCOIN PHẢI BỎ RA":"SỐ DOGCOIN PHẢI BỎ RA";skPrev()}',
    // nút khối lượng nhanh; -1 = MAX theo ví và theo trần còn lại
    'function skQ(v){if(!SKS)return;if(SKM!==1)skMode(1);',
    'if(v<0){var e=(SKS.pos&&SKS.pos.side==="short")?SKS.bid:SKS.ask;',
    'var byBal=Math.floor(SKS.balance/e),byPer=SKS.maxPer-(SKS.pos?SKS.pos.shares:0),bySan=SKS.maxShares-SKS.outstanding;',
    'var sh=Math.max(0,Math.min(byBal,byPer,bySan));v=Math.floor(sh/skLot()*10)/10;if(v<0.1){toast("Không đủ vào lệnh nhỏ nhất 0.1");return}}',
    '$("skInp").value=v;skPrev()}',
    // Đồ thị NẾN: mỗi nhịp 30s một cây, xanh = đóng cao hơn mở. Thân vẽ bằng rect,
    // bóng trên/dưới bằng line; thêm kẻ mốc gốc và kẻ giá vốn của lệnh đang mở.
    'function skChart(){var el=$("skChart");if(!el||!SKS)return;var cs=SKS.candles||[];',
    ...
    'if(SKS.base<lo)lo=SKS.base;if(SKS.base>hi)hi=SKS.base;',
    'var pad=(hi-lo)*0.12||20;lo-=pad;hi+=pad;',
    'var n=cs.length,w=300/n,bw=Math.max(1.4,w*0.6);',
    'var Y=function(v){return 142-(v-lo)/(hi-lo)*134};',
    'var g="",by=Y(SKS.base).toFixed(1);',
    ...
    'if(SKS.pos){var ya=Y(SKS.pos.avg);if(ya>2&&ya<148){',
    'g+=\'<line x1="0" y1="\'+ya.toFixed(1)+\'" x2="300" y2="\'+ya.toFixed(1)+\'" stroke="#ffcf5c" stroke-width="1" stroke-dasharray="2 3"/>\'}}',
    'el.innerHTML=g}',
    // Xem trước cho CẢ HAI chiều. Đang giữ lệnh thì chỉ cho vào thêm ĐÚNG chiều đó.
    'function skPrev(){if(!SKS)return;var raw=parseFloat($("skInp").value)||0;',
    'var held=SKS.pos?SKS.pos.side:null;',
    'var mk=function(short){var e=short?SKS.bid:SKS.ask;',
    'var sh=SKM?Math.round(raw*skLot()):Math.floor(raw/e);if(sh<0)sh=0;return {e:e,sh:sh,c:sh*e}};',
    'var L=mk(false),S=mk(true),ref=held==="short"?S:L;',
    '$("skPvC").textContent=vnd(ref.c);',
    '$("skPvS").textContent=ref.sh>0?("khối lượng "+(Math.round(ref.sh/skLot()*100)/100)+" · giá vào "+vnd(ref.e)):"chọn khối lượng ở trên";',
    '$("skMoneyT").textContent=held==="short"?"SỐ DOGCOIN PHẢI CỌC":"SỐ DOGCOIN PHẢI BỎ RA";',
    'var sp=SKS.spreadPct/100;',
    'var evenL=Math.round(SKS.ask/(1-sp)),evenS=Math.round(SKS.bid*(1-sp));',
    'var even=held==="short"?evenS:evenL;',
    '$("skPvE").textContent=ref.sh<1?"-":(held?vnd(even):(vnd(evenL)+" ↑ MUA / "+vnd(evenS)+" ↓ BÁN"));',
    'var note="";',
    'if(ref.sh>0&&!SKM){var du=raw-ref.c;if(du>0)note="Dư "+vnd(du)+" không đủ thêm khối lượng nên giữ lại trong ví. ";}',
    'if(ref.sh>0)note+=held==="short"?("Giá phải xuống "+(Math.round((1-even/SKS.price)*1000)/10)+"% bạn mới có lãi.")',
    ...
    'var setB=function(id,o,lbl,sd){var b=$(id);if(!b)return;',
    'var wrong=held&&held!==sd;',
    'b.disabled=SKBUSY||!!lock||wrong||o.sh<1||o.c>SKS.balance;',
    'b.textContent=lock?(lbl+" · "+lock):wrong?(lbl+" · đóng lệnh cũ trước")',
    ':o.sh<1?lbl:o.c>SKS.balance?(lbl+" · thiếu Dogcoin"):(lbl+" · "+vnd(o.c))};',
    'setB("skBuyBtn",L,"🟢 MUA · giá LÊN","long");setB("skSellOpenBtn",S,"🔴 BÁN · giá XUỐNG","short")}',
    // short=1 là vào lệnh BÁN (ăn khi giá xuống)
    'function skOpen(short){if(!SKS||SKBUSY)return;var raw=parseFloat($("skInp").value)||0;',
    'if(raw<=0)return toast("Chọn khối lượng đã");',
    'SKBUSY=true;skPrev();var body=SKM?{lot:raw}:{amount:Math.floor(raw)};body.side=short?"short":"long";',
    'api("/api/stock/open",body).then(function(j){SKBUSY=false;SKS=j;setBal(j.balance);$("skInp").value="";',
    ...
    '$("skSpr").textContent=SKS.spreadPct;',
    ...
    '$("skRange").textContent=(SKS.candles||[]).length+" nến · "+Math.round((SKS.candles||[]).length*SKS.tickMs/60000)+" phút";',
    'skChart();',
    ...
    '+"khối lượng "+(Math.round(p.shares/skLot()*100)/100)+" · vào <b>"+vnd(p.avg)+"</b> → nay <b>"+vnd(sht?SKS.ask:SKS.bid)+"</b> · <b style=\\"color:"+(win?"var(--green)":"var(--red)")+"\\">"+(win?"+":"")+p.plPct+"%</b>";',
    '$("skPosVal").textContent=vnd(p.value);',
    '$("skPeak").textContent=p.peak>0?("+"+vnd(p.peak)):"chưa từng có lãi";',
    'var br=$("skBurnRow");br.style.display=sht?"flex":"none";if(sht)$("skBurn").textContent=vnd(p.burnAt);',
    '$("skSellBtn").textContent=(win?"CHỐT LÃI - ĐÓNG LỆNH → ":"CẮT LỖ - ĐÓNG LỆNH → ")+vnd(p.value);',
    ...
    'var mn=SKS.mine||[];',
    '$("skMine").innerHTML=mn.length?(mn.map(function(c){var w=c.pl>=0,sh=c.side==="short";',
    'return \'<div class="skrow"><span><span class="skside \'+(sh?"s":"l")+\'">\'+(sh?"BÁN":"MUA")+\'</span>\'+(Math.round(c.shares/skLot()*100)/100)+\' · \'+vnd(c.avg)+\' → \'+vnd(c.price)+(c.forced?\' <b style="color:var(--red)">CHÁY</b>\':"")+\'</span><b style="color:\'+(w?"var(--green)":"var(--red)")+\'">\'+(w?"+":"")+vnd(c.pl)+\'</b></div>\'}).join("")',
    ...
    'skPrev()}',
    // đếm ngược tới nến kế + tự nạp lại đúng lúc nến đóng
