...
const PAGE = [
    ...
    'var ISG=[["important","⭐ QUAN TRỌNG"],["weapon","🗡️ VŨ KHÍ"],["armor","🛡️ GIÁP"],["consume","🧪 VẬT PHẨM TIÊU HAO"],["accessory","💍 PHỤ KIỆN"],["food","🍖 THỨC ĂN"],["ammo","🔫 ĐẠN"],["material","🧱 NGUYÊN LIỆU"],["implant","🧬 IMPLANT"]];',   // 09/09 +2 nhóm · 10/09 +material +implant
    ...
    'function isOnceBought(it){return !!(IS&&it.cat==="important"&&(IS.once||[]).indexOf(it.id)>=0)}',
    ...
    'function isCard(it){return "<div class=\\"isItem"+isTierCls(it)+"\\">"+isImg(it.img)+"<div class=\\"isMeta\\"><div class=\\"isNm\\">"+esc(it.name)+"</div><div class=\\"isPr\\">"+vnd(it.price)+" Dogcoin / cái</div>"+(it.note?"<div class=\\"isNote\\">"+esc(it.note)+"</div>":"")+isDayLine(it)+"</div>"',
    '+isBuyRow(it)+"</div>"}',
    // ⭐ 11/09: nhóm QUAN TRỌNG mua 1 lần/người -> không ô số lượng; đã mua -> nút "✅ ĐÃ MUA" khoá
    'function isBuyRow(it){if(it.cat==="important"){return isOnceBought(it)?"<div class=\\"isBuyRow\\"><button disabled>✅ ĐÃ MUA (1 lần/người)</button></div>":"<div class=\\"isBuyRow\\"><button onclick=\\"isBuy(\'"+it.id+"\')\\">🛒 Mua (1 lần duy nhất)</button></div>"}',
    ...
    'function isAmmoLeft(){return IS&&IS.ammoMax>0?Math.max(0,IS.ammoMax-(IS.ammoToday||0)):-1}',
    'function isMatLeft(){return IS&&IS.matMax>0?Math.max(0,IS.matMax-(IS.matToday||0)):-1}',
    'function isWtLeft(){return IS&&IS.wtMax>0?Math.max(0,IS.wtMax-(IS.wtToday||0)):-1}',
    'function isOnceLine(it){if(!IS||it.cat!=="important")return "";return isOnceBought(it)?"<div class=\\"isNote\\" style=\\"color:#8fd18f\\">✅ Bạn đã mua món này - mỗi người chỉ 1 lần</div>":"<div class=\\"isNote\\" style=\\"color:#ffd76a\\">⭐ Mỗi người chỉ mua được 1 lần duy nhất</div>"}',
    'function isImpLine(it){if(IS&&it.cat==="material"){if(!(IS.matMax>0))return isOnceLine(it);var mm=isMatLeft();return "<div class=\\"isNote\\" style=\\"color:"+(mm?"#8fd18f":"#ff8a80")+"\\">🧱 "+(mm?"hôm nay bạn còn mua được "+mm.toLocaleString()+"/"+IS.matMax.toLocaleString()+" nguyên liệu (mọi loại gộp)":"hôm nay bạn đã mua đủ "+IS.matMax.toLocaleString()+" nguyên liệu - mai quay lại")+"</div>"}if(IS&&it.cat==="ammo"){if(!(IS.ammoMax>0))return isOnceLine(it);var a=isAmmoLeft();return "<div class=\\"isNote\\" style=\\"color:"+(a?"#8fd18f":"#ff8a80")+"\\">🔫 "+(a?"hôm nay bạn còn mua được "+a.toLocaleString()+"/"+IS.ammoMax.toLocaleString()+" viên đạn (mọi loại gộp)":"hôm nay bạn đã mua đủ "+IS.ammoMax.toLocaleString()+" viên đạn - mai quay lại")+"</div>"}if(!IS||it.cat!=="implant")return isOnceLine(it);if(isWT(it)){if(!(IS.wtMax>0))return "";var w=isWtLeft();return "<div class=\\"isNote\\" style=\\"color:"+(w?"#8fd18f":"#ff8a80")+"\\">🌳 "+(w?"hôm nay bạn còn mua được "+w+"/"+IS.wtMax+" implant Cây Thế Giới":"hôm nay bạn đã mua đủ "+IS.wtMax+" implant Cây Thế Giới - mai quay lại")+"</div>"}if(!(IS.implantMax>0))return "";var l=isImpLeft();return "<div class=\\"isNote\\" style=\\"color:"+(l?"#8fd18f":"#ff8a80")+"\\">🧬 "+(l?"hôm nay bạn còn mua được "+l+"/"+IS.implantMax+" implant (mọi loại gộp)":"hôm nay bạn đã mua đủ "+IS.implantMax+" implant - mai quay lại")+"</div>"}',
    'function isDayLine(it){var imp=isImpLine(it);if(!IS||!(IS.dayMax>0)||it.cat==="implant"||it.cat==="ammo"||it.cat==="material"||it.cat==="important")return imp;var l=isDayLeft(it.id),sv=IS.dayMode!=="user";return imp+"<div class=\\"isNote\\" style=\\"color:"+(l?"#8fd18f":"#ff8a80")+"\\">📅 "+(l?(sv?"cả server hôm nay còn ":"hôm nay bạn còn mua được ")+l+"/"+IS.dayMax:(sv?"cả server đã mua hết "+IS.dayMax+" hôm nay":"hôm nay bạn đã mua đủ "+IS.dayMax)+" - mai quay lại")+"</div>"}',
    ...
    'if(it.cat==="important"&&isOnceBought(it))return toast("⭐ Bạn đã mua món này rồi - mỗi người chỉ 1 lần");',
    'var q=it.cat==="important"?1:(parseInt(($("isq_"+id)||{}).value)||0);if(q<1)return toast("Nhập số lượng");if(q>it.max)return toast("Tối đa "+it.max+"/lần");if(isWT(it)){var wl=isWtLeft();if(wl>=0&&q>wl)return toast(wl?"🌳 Hôm nay bạn còn mua được "+wl+" implant Cây Thế Giới":"🌳 Hôm nay bạn đã mua đủ "+IS.wtMax+" implant Cây Thế Giới - mai quay lại")}else if(it.cat==="implant"){var il=isImpLeft();if(il>=0&&q>il)return toast(il?"🧬 Hôm nay bạn còn mua được "+il+" implant (mọi loại gộp)":"🧬 Hôm nay bạn đã mua đủ "+IS.implantMax+" implant - mai quay lại")}else if(it.cat==="ammo"){var al=isAmmoLeft();if(al>=0&&q>al)return toast(al?"🔫 Hôm nay bạn còn mua được "+al.toLocaleString()+" viên đạn (mọi loại gộp)":"🔫 Hôm nay bạn đã mua đủ "+IS.ammoMax.toLocaleString()+" viên đạn - mai quay lại")}else if(it.cat==="material"){var ml=isMatLeft();if(ml>=0&&q>ml)return toast(ml?"🧱 Hôm nay bạn còn mua được "+ml.toLocaleString()+" nguyên liệu (mọi loại gộp)":"🧱 Hôm nay bạn đã mua đủ "+IS.matMax.toLocaleString()+" nguyên liệu - mai quay lại")}var dl=(it.cat==="implant"||it.cat==="ammo"||it.cat==="material")?-1:isDayLeft(id);if(dl>=0&&q>dl)return toast(dl?"📅 Hôm nay "+(IS.dayMode!=="user"?"cả server":"bạn")+" còn mua được "+dl+" "+it.name:"📅 Hôm nay "+(IS.dayMode!=="user"?"cả server":"bạn")+" đã mua đủ "+IS.dayMax+" "+it.name+" - mai quay lại");',
    ...
    'var na=parseInt(($("dogNapAmt")||{}).value)||0,np=$("dogNapPrev");if(np){if(!na)np.textContent="";else{var web=Math.floor(na*DOGRATE),nlft=dm?Math.max(0,dm-(DOGST.napToday||0)):Infinity;if(mx&&na>mx){np.style.color="#ff8a80";np.textContent="⚠️ Vượt trần "+vnd(mx)+"/lần"}else if(na>nlft){np.style.color="#ff8a80";np.textContent="⚠️ Vượt hạn ngày: hôm nay chỉ còn lấy được "+vnd(nlft)+" Dogcoin trong game (= nhận "+vnd(Math.floor(nlft*DOGRATE))+" web)"}else{np.style.color="#8fd18f";np.textContent="→ Lấy "+vnd(na)+" Dogcoin trong game, ví web +"+vnd(web)+(DOGRATE!==1?" (tỉ lệ 1 : "+DOGRATE+")":"")}}}}',
