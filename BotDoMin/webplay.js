...
const PAGE = [
    ...
    '<div id="skHelpBody">',
    '<ol>',
    '<li>Giá DOG <b>nhảy 5 giây một lần</b>, cây nến cuối lớn dần theo giá và <b>cứ 40 giây chốt thành một cây</b>. Nến <b style="color:var(--green)">xanh</b> là giá lên, <b style="color:var(--red)">đỏ</b> là giá xuống.</li>',
    ...
    '<li>Điền <b>số Dogcoin</b> muốn xuống (hoặc bấm 25% / 50% / 75% / TẤT TAY theo ví). Bot tự quy ra khối lượng và hiện đúng số tiền sẽ bị trừ.</li>',
    ...
    '<div class="muted" style="font-size:11.5px;margin-top:2px">chốt nến sau <b id="skNext">-</b> · giá nhảy mỗi <b>5s</b> · mốc gốc <b id="skBase">1.000</b></div></div>',
    '<div id="skOhlc">',
    ...
    '</div>',
    '<div id="skTf">',
    '<button id="sktf1" class="on" onclick="skTf(1)">40s</button>',
    '<button id="sktf3" onclick="skTf(3)">2m</button>',
    '<button id="sktf9" onclick="skTf(9)">6m</button>',
    '<button id="sktf30" onclick="skTf(30)">20m</button>',
    ...
    '[1,3,9,30].forEach(function(k){var b=$("sktf"+k);if(b)b.classList.toggle("on",k===n)});skRender()}',
    ...
    'var cap=Math.floor(SKS.balance*p/100);',
    ...
    'var m=Math.min(cap,shCap*skRef());',
    'if(m<skRef())return toast("Không đủ vào lệnh nhỏ nhất ("+vnd(skRef())+" Dogcoin)");',
    '$("skMoney").value=Math.floor(m);skPrev()}',
    ...
    // mỗi chiều vào ở giá khác nhau -> số CP mua được cũng khác, tính riêng từng chiều
    'var mk=function(e){var sh=Math.floor(m/e);if(sh<0)sh=0;return {e:e,sh:sh,c:sh*e}};',
    ...
    'var kl=$("skPvKl");if(kl)kl.textContent=sh<1?"-":((Math.round(sh/skLot()*100)/100)+" ("+sh+" CP)");',
    'var note="";',
    'if(sh>0&&m>ref.c)note="Dư "+vnd(m-ref.c)+" không đủ thêm khối lượng nên giữ lại trong ví. ";',
    ...
    ':o.sh<1?(lbl+" · nhập tiền đã"):o.c>SKS.balance?(lbl+" · thiếu Dogcoin"):(lbl+" · "+vnd(o.c))};',
    ...
    'SKBUSY=true;skPrev();',
    'api("/api/stock/open",{amount:m,side:short?"short":"long"}).then(function(j){SKBUSY=false;SKS=j;',
    ...
    'pctAt(8,"skP5");pctAt(45,"skP30");pctAt(rc.length,"skP120");',
    ...
    'skPrev()}',
    // Đồng hồ đếm tới lúc CHỐT NẾN (40s). Giá nhảy mỗi 5 giây nên nạp lại mỗi 5 giây
    ...
    'if(left<0)left=0;$("skNext").textContent=left+"s"},1000);',
    'setInterval(function(){if(TOKEN&&!SKBUSY&&localStorage.getItem("play_page")==="stock")skSync()},5000);',
    ...
    '[1,3,9,30].forEach(function(k){var q=$("sktf"+k);if(q)q.classList.toggle("on",k===SKTF)});',
