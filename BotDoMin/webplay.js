...
const PAGE = [
    ...
    '<div id="skHelpBody">',
    '<ol>',
    '<li>Giá DOG <b>nhảy 30 giây một lần</b>, mỗi lần vẽ thêm một cây nến. Nến <b style="color:var(--green)">xanh</b> là giá lên, <b style="color:var(--red)">đỏ</b> là giá xuống.</li>',
    ...
    '<li>Trước khi bấm phải điền <b>số Dogcoin</b> muốn xuống <b>và</b> <b>khối lượng</b>. Sửa ô nào ô kia tự tính, khỏi bấm máy tính.</li>',
    ...
    '</ol>',
    '<div class="skex"><div class="l">Ví dụ MUA khối lượng 0.1 — trừ ví 1.020 Dogcoin</div>',
    '<div class="muted" style="margin-top:3px">Giá lên 1.100 → đóng lệnh nhận <b style="color:var(--green)">1.078</b> → <b>lãi 58</b><br>',
    'Giá xuống 950 → đóng lệnh nhận <b style="color:var(--red)">931</b> → <b>lỗ 89</b></div></div>',
    '<div class="skex"><div class="s">Ví dụ BÁN khối lượng 0.1 — cọc 980 Dogcoin</div>',
    '<div class="muted" style="margin-top:3px">Giá xuống 900 → đóng lệnh nhận <b style="color:var(--green)">1.042</b> → <b>lãi 62</b><br>',
    'Giá lên 1.100 → đóng lệnh nhận <b style="color:var(--red)">838</b> → <b>lỗ 142</b></div></div>',
    '<div class="muted" style="font-size:12px;margin-top:9px">⚠️ Vào lệnh là mất ngay <b>4%</b> tiền chênh mua–bán (phí sàn), nên giá phải nhích đủ 4% bạn mới bắt đầu có lãi — xem dòng <b>Hoà vốn ở giá</b>. Lệnh <b>BÁN</b> còn <b>cháy cọc</b> nếu giá lên gấp đôi giá vào: mất hết cọc và không mất thêm.</div>',
    '</div></div>',
    ...
    '<div class="muted" style="font-size:11.5px;margin-top:2px">nến sau <b id="skNext">-</b> · mốc gốc <b id="skBase">1.000</b></div></div>',
    '<div id="skOhlc">',
    ...
    '</div>',
    '<div id="skTf">',
    '<button id="sktf1" class="on" onclick="skTf(1)">30s</button>',
    '<button id="sktf4" onclick="skTf(4)">2m</button>',
    '<button id="sktf10" onclick="skTf(10)">5m</button>',
    '<button id="sktf30" onclick="skTf(30)">15m</button>',
    ...
    '<div><div class="t">2 giờ</div><div class="v" id="skP120">-</div></div>',
    '</div>',
    ...
    '<div id="skPair">',
    '<div><label>SỐ DOGCOIN</label><input id="skMoney" type="number" min="1" placeholder="vd 10.000" oninput="skFromMoney()"></div>',
    '<div><label>KHỐI LƯỢNG</label><input id="skVol" type="number" step="0.1" min="0.1" placeholder="vd 0.1" oninput="skFromVol()"></div>',
    '</div>',
    ...
    '<div class="muted" style="font-size:11.5px;margin-top:8px">Chọn một chiều, muốn đổi thì đóng lệnh cũ trước. Lệnh <b>BÁN</b> phải cọc đúng bằng giá trị lệnh. Chênh mua–bán <b id="skSpr">2</b>% mỗi chiều là phí sàn. Còn <b id="skLeft">-</b> khối lượng, mỗi người tối đa <b id="skPer">-</b>.</div>',
    '</div>',
    ...
    '[1,4,10,30].forEach(function(k){var b=$("sktf"+k);if(b)b.classList.toggle("on",k===n)});skRender()}',
    ...
    // HAI Ô LIÊN ĐỘNG: sửa số Dogcoin -> tính khối lượng, sửa khối lượng -> tính tiền.
    // Khối lượng luôn quy về số CP nguyên nên tiền hiện ra là tiền THẬT sẽ bị trừ.
    'function skFromMoney(){if(!SKS)return;var m=parseInt($("skMoney").value)||0;',
    'var sh=Math.floor(m/skRef());if(sh<0)sh=0;',
    '$("skVol").value=sh>0?(Math.round(sh/skLot()*100)/100):"";skPrev()}',
    'function skFromVol(){if(!SKS)return;var v=parseFloat($("skVol").value)||0;',
    'var sh=Math.round(v*skLot());if(sh<0)sh=0;',
    '$("skMoney").value=sh>0?(sh*skRef()):"";skPrev()}',
    'function skQ(v){if(!SKS)return;',
    'if(v<0){var byBal=Math.floor(SKS.balance/skRef());',
    'var byPer=SKS.maxPer-(SKS.pos?SKS.pos.shares:0),bySan=SKS.maxShares-SKS.outstanding;',
    'var sh=Math.max(0,Math.min(byBal,byPer,bySan));v=Math.floor(sh/skLot()*10)/10;',
    'if(v<0.1)return toast("Không đủ vào khối lượng nhỏ nhất 0.1")}',
    '$("skVol").value=v;skFromVol()}',
    ...
    'function skPrev(){if(!SKS)return;var sh=Math.round((parseFloat($("skVol").value)||0)*skLot());',
    'if(sh<0)sh=0;var held=SKS.pos?SKS.pos.side:null;',
    'var L={e:SKS.ask,c:sh*SKS.ask},S={e:SKS.bid,c:sh*SKS.bid};',
    'var ref=held==="short"?S:L;',
    ...
    'b.disabled=SKBUSY||!!lock||wrong||sh<1||o.c>SKS.balance;',
    'b.textContent=lock?(lbl+" · "+lock):wrong?(lbl+" · đóng lệnh cũ trước")',
    ':sh<1?(lbl+" · điền số đã"):o.c>SKS.balance?(lbl+" · thiếu Dogcoin"):(lbl+" · "+vnd(o.c))};',
    'setB("skBuyBtn",L,"🟢 MUA · giá LÊN","long");setB("skSellOpenBtn",S,"🔴 BÁN · giá XUỐNG","short")}',
    'function skOpen(short){if(!SKS||SKBUSY)return;var v=parseFloat($("skVol").value)||0;',
    'if(v<=0)return toast("Điền số Dogcoin hoặc khối lượng đã");',
    'SKBUSY=true;skPrev();',
    'api("/api/stock/open",{lot:v,side:short?"short":"long"}).then(function(j){SKBUSY=false;SKS=j;',
    'setBal(j.balance);$("skVol").value="";$("skMoney").value="";',
    ...
    'pctAt(10,"skP5");pctAt(60,"skP30");pctAt(rc.length,"skP120");',
    ...
    '[1,4,10,30].forEach(function(k){var q=$("sktf"+k);if(q)q.classList.toggle("on",k===SKTF)});',
