...
⚠️ **Tổng mỗi bảng phải đúng 1.00.** `spinWheel` trừ dần → thiếu bao nhiêu là **dồn hết vào ô CUỐI bảng** không báo gì. `luckywheeltest.js` canh. Hạ nổ hũ thì phần dôi ra dồn vào ô "hãm" (Dò Mìn: 🍂 Hụt; Leo Thang không có Hụt nên vào ↩️ Hoàn vé). Dò Mìn: 48,5% số hộp là hụt hoặc chỉ hoàn phí — cố ý hãm mạnh; muốn nới thì kéo 2 ô đó xuống bù cho 💰/⛏️. Đổi phí cỏ thì sửa **cả 2** dòng `fee` trong `index.js` và mọi chữ "40% cược" + phép `*0.4` ở `webplay.js`.
