...
6. **`logDog` chỉ ghi khoản CHUYỂN / ĐIỀU CHỈNH** (admin cộng trừ, chuyển giữa người chơi, nạp/rút vào ra game, mua pal, vay/trả nợ, hoàn). **22/09 chủ server chốt: KHÔNG ghi bất cứ gì của mini game**, `DOG_LEDGER_BO_QUA` (`bet · jackpot · cophieu · tienlen · sieutx · roulette`) chặn ở cửa ghi và `getDogLedger` lọc luôn dòng cũ. Từng ván thắng thua tra ở tab 📜 LOG (mục riêng từng trò, Siêu Tài Xỉu có mục riêng). Bộ kiểm: `node TaiXiu/kiemtra/log-test.js`.
...
### 🎡 Roulette (thuần web, `../Roulette/`), bàn thứ ba, 154 cửa, nhân CHỈ rơi vào số đơn
Dựng theo đúng ảnh "Cách chơi" chủ server gửi 25/09: vòng trong **Trực tiếp · Tác (2 số) · Dãy (3) · Góc (4) · Hàng (6)**, vòng ngoài **Cột · Tá · Đỏ/Đen · Chẵn/Lẻ · 1-18/19-36**. Chip đặt được ở giao điểm (góc, tác, dãy, hàng) nhưng **hệ số nhân chỉ rơi vào SỐ ĐƠN** ("chip đặt được ở ngã 4 chứ không phải nhân ở ngã 4"). Tiền nuôi hệ số nhân lấy từ chính số đơn: trả **29:1** thay vì 35:1, phần thiếu do nhân (thang 7 bậc x50 tới x500, sát Lightning Roulette thật) bù; mọi cửa khác trả chuẩn, RTP gốc đều đúng 36/37. Số ô sét mỗi ván bốc theo **bảng tỉ lệ 2..7 ô** (2 hay gặp, 7 hiếm, trung bình ≈4,4), máy nghiêng bảng cho khớp thang nên nhà cái vẫn đúng mức, admin đổi ở panel (`/api/rl/set`, DB `_rlSet`). Nhà cái ăn qua **một mức phí duy nhất** suy từ mức ăn: `phí = (36/37)/(1 − ăn) − 1`, ăn 8% thì phí 5,76%, đặt 10.000 trừ 10.575. Sàn mức ăn **3%** (dưới 2,7027% là phí âm, bàn tự phát tiền), trần 20%. Trần cược theo nhóm: số đơn 5.000 (vì x500), còn lại quanh 300.000 thắng đậm nhất. Máy bàn `Roulette/ban.js` (`taoBan(ctx)` + nhịp 1 giây), lõi tiền `Roulette/cua.js`; DB `_rl*`; web `/api/rl/*`; panel `/api/rl/*` (chỉ SUPER). Nhịp ván **15s đặt + 5s bi chạy vẫn đặt + 14s khoá** (kết quả và số sét chốt NGAY lúc khoá để web vẽ bi rơi đúng ô, +2s mới hiện nhân, bi giảm tốc liền mạch rồi rơi ở giây 11, 3s cuối khoe kết quả và cắm dolly lên ô thắng). Hoàn tiền dùng `b.phi` **đã ghi trên phiếu**, không tính lại (admin đổi mức ăn giữa ván thì tính lại sẽ hoàn sai). Bộ kiểm: `node Roulette/kiemtra/cua-test.js` (103) và `node Roulette/kiemtra/ban-test.js` (76). Luật + cạm bẫy: `../Roulette/README.md`. Chưa có bảng Discord.
